 ...
 * whole system.
 *
 * <p><strong>The host matches {@code major.minor} exactly.</strong> A plugin declaring {@code 0.5.x} is
 * rejected the moment the host runs {@code 0.6.0} — there is no forward or backward tolerance. While the
 * SDK is pre-1.0 a breaking change is therefore a <em>minor</em> bump ({@code 0.5.0} → {@code 0.6.0});
